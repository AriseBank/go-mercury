...
type Container struct {
	container *C.struct_mercury_container
	mu        sync.RWMutex
...
const (
	...
	isPrivileged
	isUnprivileged
	isGreaterEqualThanMERCURY11
	isGreaterEqualThanMERCURY20
)
...
func (c *Container) makeSure(flags int) error {
	...
	if flags&isGreaterEqualThanMERCURY11 != 0 && !VersionAtLeast(1, 1, 0) {
		return ErrNotSupported
	}

	if flags&isGreaterEqualThanMERCURY20 != 0 && !VersionAtLeast(2, 0, 0) {
		return ErrNotSupported
	}
...
func (c *Container) Defined() bool {
	...
	return bool(C.go_mercury_defined(c.container))
}
...
func (c *Container) Running() bool {
	...
	return bool(C.go_mercury_running(c.container))
}
...
func (c *Container) Controllable() bool {
	...
	return bool(C.go_mercury_may_control(c.container))
}
...
func (c *Container) CreateSnapshot() (*Snapshot, error) {
	...
	ret := int(C.go_mercury_snapshot(c.container))
	if ret < 0 {
...
func (c *Container) RestoreSnapshot(snapshot Snapshot, name string) error {
	...
	if !bool(C.go_mercury_snapshot_restore(c.container, csnapname, cname)) {
...
func (c *Container) DestroySnapshot(snapshot Snapshot) error {
	...
	if !bool(C.go_mercury_snapshot_destroy(c.container, csnapname)) {
...
func (c *Container) DestroyAllSnapshots() error {
	if err := c.makeSure(isDefined | isGreaterEqualThanMERCURY11); err != nil {
		return err
	}
	...
	if !bool(C.go_mercury_snapshot_destroy_all(c.container)) {
...
func (c *Container) Snapshots() ([]Snapshot, error) {
	...
	var csnapshots *C.struct_mercury_snapshot

	size := int(C.go_mercury_snapshot_list(c.container, &csnapshots))
	...
	gosnapshots := *(*[]C.struct_mercury_snapshot)(unsafe.Pointer(&hdr))
	...
	for i := 0; i < size; i++ {
		snapshots[i] = Snapshot{
			...
			Path:        C.GoString(gosnapshots[i].mercurypath),
		}
	}
...
func (c *Container) State() State {
	...
	return StateMap[C.GoString(C.go_mercury_state(c.container))]
}
...
func (c *Container) InitPid() int {
	...
	return int(C.go_mercury_init_pid(c.container))
}
...
func (c *Container) WantDaemonize(state bool) error {
	...
	if !bool(C.go_mercury_want_daemonize(c.container, C.bool(state))) {
...
func (c *Container) WantCloseAllFds(state bool) error {
	...
	if !bool(C.go_mercury_want_close_all_fds(c.container, C.bool(state))) {
...
func (c *Container) Freeze() error {
	...
	if !bool(C.go_mercury_freeze(c.container)) {
...
func (c *Container) Unfreeze() error {
	...
	if !bool(C.go_mercury_unfreeze(c.container)) {
...
func (c *Container) Create(options TemplateOptions) error {
	...
	if args != nil {
		...
		ret = bool(C.go_mercury_create(c.container, ctemplate, cbackend, C.int(c.verbosity), cargs))
	} else {
		ret = bool(C.go_mercury_create(c.container, ctemplate, cbackend, C.int(c.verbosity), nil))
	}
...
func (c *Container) Start() error {
	...
	if !bool(C.go_mercury_start(c.container, 0, nil)) {
...
func (c *Container) StartWithArgs(args []string) error {
	...
	if !bool(C.go_mercury_start(c.container, 0, makeNullTerminatedArgs(args))) {
		return ErrStartFailed
	}
	return nil
}
...
func (c *Container) Execute(args ...string) ([]byte, error) {
	...
	cargs := []string{"mercury-execute", "-n", c.Name(), "-P", c.ConfigPath(), "--"}
	cargs = append(cargs, args...)

	c.mu.Lock()
	defer c.mu.Unlock()

	// FIXME: Go runtime and src/mercury/start.c signal_handler are not playing nice together so use mercury-execute for now
	...
	/*
		...
		if !bool(C.go_mercury_start(c.container, 1, cargs)) {
...
func (c *Container) Stop() error {
	...
	if !bool(C.go_mercury_stop(c.container)) {
...
func (c *Container) Reboot() error {
	...
	if !bool(C.go_mercury_reboot(c.container)) {
...
func (c *Container) Shutdown(timeout time.Duration) error {
	...
	if !bool(C.go_mercury_shutdown(c.container, C.int(timeout.Seconds()))) {
...
func (c *Container) Destroy() error {
	...
	if !bool(C.go_mercury_destroy(c.container)) {
...
func (c *Container) DestroyWithAllSnapshots() error {
	if err := c.makeSure(isDefined | isNotRunning | isGreaterEqualThanMERCURY11); err != nil {
		return err
	}
	...
	if !bool(C.go_mercury_destroy_with_snapshots(c.container)) {
...
func (c *Container) Clone(name string, options CloneOptions) error {
	...
	var flags int
	if options.KeepName {
		flags |= C.MERCURY_CLONE_KEEPNAME
	}
	if options.KeepMAC {
		flags |= C.MERCURY_CLONE_KEEPMACADDR
	}
	if options.Snapshot {
		flags |= C.MERCURY_CLONE_SNAPSHOT
	}
	...
	if options.ConfigPath != "" {
		cmercurypath := C.CString(options.ConfigPath)
		defer C.free(unsafe.Pointer(cmercurypath))

		if !bool(C.go_mercury_clone(c.container, cname, cmercurypath, C.int(flags), cbackend)) {
			return ErrCloneFailed
		}
	} else {
		if !bool(C.go_mercury_clone(c.container, cname, nil, C.int(flags), cbackend)) {
			return ErrCloneFailed
		}
	}
	return nil
...
func (c *Container) Rename(name string) error {
	...
	if !bool(C.go_mercury_rename(c.container, cname)) {
...
func (c *Container) Wait(state State, timeout time.Duration) bool {
	...
	return bool(C.go_mercury_wait(c.container, cstate, C.int(timeout.Seconds())))
}
...
func (c *Container) ConfigFileName() string {
	...
	// allocated in mercury.c
	configFileName := C.go_mercury_config_file_name(c.container)
...
func (c *Container) ConfigItem(key string) []string {
	...
	// allocated in mercury.c
	configItem := C.go_mercury_get_config_item(c.container, ckey)
...
func (c *Container) SetConfigItem(key string, value string) error {
	...
	if !bool(C.go_mercury_set_config_item(c.container, ckey, cvalue)) {
...
func (c *Container) RunningConfigItem(key string) []string {
	...
	// allocated in mercury.c
	configItem := C.go_mercury_get_running_config_item(c.container, ckey)
	defer C.free(unsafe.Pointer(configItem))
...
func (c *Container) CgroupItem(key string) []string {
	...
	// allocated in mercury.c
	cgroupItem := C.go_mercury_get_cgroup_item(c.container, ckey)
...
func (c *Container) SetCgroupItem(key string, value string) error {
	...
	if !bool(C.go_mercury_set_cgroup_item(c.container, ckey, cvalue)) {
...
func (c *Container) ClearConfig() {
	...
	C.go_mercury_clear_config(c.container)
}
...
func (c *Container) ClearConfigItem(key string) error {
	...
	if !bool(C.go_mercury_clear_config_item(c.container, ckey)) {
...
func (c *Container) ConfigKeys(key ...string) []string {
	...
	if key != nil && len(key) == 1 {
		...
		// allocated in mercury.c
		keys = C.go_mercury_get_keys(c.container, ckey)
		defer C.free(unsafe.Pointer(keys))
	} else {
		// allocated in mercury.c
		keys = C.go_mercury_get_keys(c.container, nil)
		defer C.free(unsafe.Pointer(keys))
	}
...
func (c *Container) LoadConfigFile(path string) error {
	...
	if !bool(C.go_mercury_load_config(c.container, cpath)) {
...
func (c *Container) SaveConfigFile(path string) error {
	...
	if !bool(C.go_mercury_save_config(c.container, cpath)) {
...
func (c *Container) ConfigPath() string {
	...
	return C.GoString(C.go_mercury_get_config_path(c.container))
}
...
func (c *Container) SetConfigPath(path string) error {
	...
	if !bool(C.go_mercury_set_config_path(c.container, cpath)) {
...
func (c *Container) ConsoleFd(ttynum int) (int, error) {
	...
	ret := int(C.go_mercury_console_getfd(c.container, C.int(ttynum)))
	if ret < 0 {
...
func (c *Container) Console(options ConsoleOptions) error {
	...
	ret := bool(C.go_mercury_console(c.container,
		C.int(options.Tty),
...
func (c *Container) AttachShell(options AttachOptions) error {
	...
	ret := int(C.go_mercury_attach(c.container,
...
func (c *Container) RunCommandStatus(args []string, options AttachOptions) (int, error) {
	...
	ret := int(C.go_mercury_attach_run_wait(
		c.container,
		C.bool(options.ClearEnv),
		C.int(options.Namespaces),
		C.long(options.Arch),
...
func (c *Container) RunCommandNoWait(args []string, options AttachOptions) (int, error) {
	...
	ret := int(C.go_mercury_attach_no_wait(
		c.container,
		C.bool(options.ClearEnv),
		C.int(options.Namespaces),
		C.long(options.Arch),
...
func (c *Container) Interfaces() ([]string, error) {
	...
	result := C.go_mercury_get_interfaces(c.container)
	if result == nil {
...
func (c *Container) InterfaceStats() (map[string]map[string]ByteSize, error) {
	...
	netPrefix := "mercury.net"
	if !VersionAtLeast(2, 1, 0) {
		netPrefix = "mercury.network"
	}
...
func (c *Container) IPAddress(interfaceName string) ([]string, error) {
	...
	result := C.go_mercury_get_ips(c.container, cinterface, nil, 0)
	if result == nil {
...
func (c *Container) IPv4Address(interfaceName string) ([]string, error) {
	...
	result := C.go_mercury_get_ips(c.container, cinterface, cfamily, 0)
	if result == nil {
...
func (c *Container) IPAddresses() ([]string, error) {
	...
	result := C.go_mercury_get_ips(c.container, nil, nil, 0)
	if result == nil {
		return nil, ErrIPAddresses
	}
	return convertArgs(result), nil
...
func (c *Container) IPv4Addresses() ([]string, error) {
	...
	result := C.go_mercury_get_ips(c.container, nil, cfamily, 0)
	if result == nil {
		return nil, ErrIPv4Addresses
	}
	return convertArgs(result), nil
...
func (c *Container) LogFile() string {
	if VersionAtLeast(2, 1, 0) {
		return c.ConfigItem("mercury.log.file")[0]
	}

	return c.ConfigItem("mercury.logfile")[0]
}
...
func (c *Container) SetLogFile(filename string) error {
	var err error
	if VersionAtLeast(2, 1, 0) {
		err = c.SetConfigItem("mercury.log.file", filename)
	} else {
		err = c.SetConfigItem("mercury.logfile", filename)
	}
	if err != nil {
		return err
	}
...
func (c *Container) LogLevel() LogLevel {
	if VersionAtLeast(2, 1, 0) {
		return logLevelMap[c.ConfigItem("mercury.log.level")[0]]
	}

	return logLevelMap[c.ConfigItem("mercury.loglevel")[0]]
}
...
func (c *Container) SetLogLevel(level LogLevel) error {
	var err error
	if VersionAtLeast(2, 1, 0) {
		err = c.SetConfigItem("mercury.log.level", level.String())
	} else {
		err = c.SetConfigItem("mercury.loglevel", level.String())
	}
	if err != nil {
		return err
	}
...
func (c *Container) AddDeviceNode(source string, destination ...string) error {
	...
	if destination != nil && len(destination) == 1 {
		...
		if !bool(C.go_mercury_add_device_node(c.container, csource, cdestination)) {
			return ErrAddDeviceNodeFailed
		}
		return nil
	}

	if !bool(C.go_mercury_add_device_node(c.container, csource, nil)) {
		return ErrAddDeviceNodeFailed
	}
	return nil
...
func (c *Container) RemoveDeviceNode(source string, destination ...string) error {
	...
	if destination != nil && len(destination) == 1 {
		...
		if !bool(C.go_mercury_remove_device_node(c.container, csource, cdestination)) {
			return ErrRemoveDeviceNodeFailed
		}
		return nil
	}

	if !bool(C.go_mercury_remove_device_node(c.container, csource, nil)) {
		return ErrRemoveDeviceNodeFailed
	}
	return nil
}
...
func (c *Container) Checkpoint(opts CheckpointOptions) error {
	if err := c.makeSure(isRunning | isGreaterEqualThanMERCURY11); err != nil {
		return err
	}
	...
	if !C.go_mercury_checkpoint(c.container, cdirectory, cstop, cverbose) {
...
func (c *Container) Restore(opts RestoreOptions) error {
	if err := c.makeSure(isGreaterEqualThanMERCURY11); err != nil {
		return err
	}
	...
	if !C.bool(C.go_mercury_restore(c.container, cdirectory, cverbose)) {
...
func (c *Container) Migrate(cmd uint, opts MigrateOptions) error {
	...
	ret := C.int(C.go_mercury_migrate(c.container, C.uint(cmd), &copts, &extras))
	if ret != 0 {
...
func (c *Container) AttachInterface(source, destination string) error {
	if err := c.makeSure(isRunning | isPrivileged | isGreaterEqualThanMERCURY11); err != nil {
		return err
	}
	...
	if !bool(C.go_mercury_attach_interface(c.container, csource, cdestination)) {
...
func (c *Container) DetachInterface(source string) error {
	...
	if !bool(C.go_mercury_detach_interface(c.container, csource, nil)) {
...
func (c *Container) DetachInterfaceRename(source, target string) error {
	...
	if !bool(C.go_mercury_detach_interface(c.container, csource, ctarget)) {
		return ErrDetachInterfaceFailed
	}
	return nil
}
